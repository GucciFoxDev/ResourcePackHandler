package me.ozyrncraft.commands;
...
public class PackMCCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Check if the length of the command is greater than 1 (aka "/pack")
        if (args.length > 0) {

            // If the command is "setpack" move forward
            if (args[0].equals("setpack")) {
                // If the sender has sufficient permissions
                if (sender.hasPermission("ozyrncraft.pack.setpack")) {
                    ...
                    // Send a not enough arguments message to the player
                        ...
                        sender.sendMessage(ChatColor.RED + "You should use /pack setpack <world name> <resource pack ID> <force pack?>");
                    }
            ...
            // If the command is "status"...
                ...
                if (sender.hasPermission("ozyrncraft.pack.status")) {
            ...
            // If the command is "reload-config"...
                ...
                if (sender.hasPermission("ozyrncraft.pack.reload-config")) {
                    // Reload config and send a message to the player
                    PackHandler.getPlugin(PackHandler.class).reloadConfig();
